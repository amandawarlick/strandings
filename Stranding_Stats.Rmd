...
#Look at the timeline break either descriptively or via hypothesis testing, or loess model.
...
#slope of line =  exp(0.050462*x) = y = 1.05^x
#1.05^10 = 1.65 = ~60% change in number/decade

#Negative binomial GLM
summary(glm.nb(cnt ~ Year.of.Observation, data = data_by_year))
#slope of line =  exp(0.046800*x) = y = 1.0479^x
...
#Negative binomial
summary(glm.nb(cnt ~ Year.of.Observation, data = all_sp %>% filter(Pinniped.Common.Name == 'Harbor seal'))) # y = exp(5.802e-02x) = 1.786 fold per decade
...
summary(glm.nb(cnt ~ Year.of.Observation, data = all_sp %>% filter(Pinniped.Common.Name == 'Steller sea lion'))) # y = exp(0.13784x) = 1.1477^10 = 3.9 fold per decade?!
...
summary(glm.nb(cnt ~ Year.of.Observation, data = all_sp %>% filter(Pinniped.Common.Name == 'Northern elephant seal')))
...
#regression equation: y = exp(0.09643*x) = 1.10^x
...
summary(glm.nb(cnt ~ Year.of.Observation, data = HI_sp %>% filter(Pinniped.Common.Name == 'Harbor seal'))) # y = 1.12^x
...
summary(glm.nb(cnt ~ Year.of.Observation, data = HI_sp %>% filter(Pinniped.Common.Name == 'Northern elephant seal'))) # y = 1.02^x

summary(glm.nb(cnt ~ Year.of.Observation, data = HI_sp %>% filter(Pinniped.Common.Name == 'Guadalupe fur seal'))) #insufficient observations?
...
#For Moni: We had talked about determining whether I needed a link = identity?
summary(glm(cbind(Yes, No) ~ Year.of.Observation, family = binomial, data = HI_yes_no)) #y = exp(.06716*10) = 1.957 = ~100% increase per decade
...
summary(glm(cbind(Yes, No) ~ Year.of.Observation, family = binomial, data = HI_yes_no_sp %>% filter(Pinniped.Common.Name == 'Harbor seal'))) #y = exp(.07256*10) = 2.07 = ~200% increase per decade
...
summary(glm(cbind(Yes, No) ~ Year.of.Observation, family = binomial, data = HI_yes_no_sp %>% filter(Pinniped.Common.Name == 'Steller sea lion')))
...
summary(glm(cbind(Yes, No) ~ Year.of.Observation, family = binomial, data = HI_yes_no_sp %>% filter(Pinniped.Common.Name == 'Guadalupe fur seal')))

summary(glm(cbind(Yes, No) ~ Year.of.Observation, family = binomial, data = HI_yes_no_sp %>% filter(Pinniped.Common.Name == 'Northern fur seal')))
...
summary(glm(cbind(Boat, NoBoat) ~ Year.of.Observation, family = binomial, data = HI_type_yes_no)) # y = exp(0.08687*x) = 1.09^x
summary(glm(cbind(Fisheries, NoFish) ~ Year.of.Observation, family = binomial, data = HI_type_yes_no)) # y = exp(0.043703*x) = 1.04^x
...
#For Moni, help find more eloquent solution to see if certain interaction types are increasing or decreasing for specific species?
#GLM regressions for each species ~ interaction type, there is obviously a better way
summary(glm(cbind(Boat, NoBoat) ~ Year.of.Observation, family = binomial, data = HIcount_type_species %>% filter(Pinniped.Common.Name == 'Harbor seal')))

summary(glm(cbind(Fisheries, NoFish) ~ Year.of.Observation, family = binomial, data = HIcount_type_species %>% filter(Pinniped.Common.Name == 'Harbor seal')))

summary(glm(cbind(Gunshot, NoGun) ~ Year.of.Observation, family = binomial, data = HIcount_type_species %>% filter(Pinniped.Common.Name == 'Harbor seal')))
...
#Moni recommended glm with and without interaction, anova the two
# <--- p-value < 0.001, indicating that at least one species has significant interaction of month
...
#For Moni - is it incorrect to use glm to see the specific peak for each species?
...
#Separate species, same as above for all strandings, check with Moni if correct; 
...
# Moni suggests anova(model, model_interaction)
