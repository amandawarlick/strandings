...
ggsave(Sex.all.yr.figure.stack, file = "~/Documents/R/Strandings/Figures/Sex.all.yr.figure.stack.pdf")

Sex.all.yr.figure <- ggplot(Sex.all.yr, 
                        aes(x = as.factor(Year.of.Observation), y = cnt, fill = Sex)) +
...
Sex.all.yr.boxplot <- 
  ...
  plot_theme(legend.position = 'top')
ggsave(Sex.all.yr.boxplot, file = "~/Documents/R/Strandings/Figures/Sex.all.yr.boxplot.pdf")
...
summary(aov(cnt ~ Sex + Month.of.Observation + Month.of.Observation*Sex, Sex.all.yr.mo))
...
#HELP - need to bind the "average" row at the bottom, call the row "average", and then make sure it's not behaving strangely for the inline calls, where currently it is appending an "NA" after any percentages I call within "draftmanuscript"
...
summary(aov(cnt ~ Sex + Age.Class + Sex*Age.Class, data = age.sex.stats))
TukeyHSD(aov(cnt ~ Sex + Age.Class + Sex*Age.Class, data = age.sex.stats))
...
Sex.HI.type.table <- pinnipeds.data %>%
  ...
  select(c(Sex, All.prop, HI.prop, Prev.HI, Prop.fish, Prop.shot, Prop.boat, Prop.other))
colnames(Sex.HI.type.table) <- c("Sex", "All Strandings (%)", "Human Interactions (%)", "Prevalence of HI (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")
...
Age.HI.type.table <- pinnipeds.data %>%
  ...
  select(c(Age.Class, All.prop, HI.prop, Prev.HI, Prop.fish, Prop.shot, Prop.boat, Prop.other))
colnames(Age.HI.type.table) <- c("Age", "All Strandings (%)", "Human Interactions (%)", "Prevalence of HI (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")
...
species.prop.HI <- pinnipeds.data %>%
  ...
  select(Pinniped.Common.Name, all.prop, HI.prop, Prev.HI, Prop.Fisheries, Prop.Gunshot, Prop.Boat, Prop.Other, prop.or, prop.wa) 

species.prop.HI[is.na(species.prop.HI)] <- 0

colnames(species.prop.HI) <- c("Species", "All Strandings (%)", "Human Interactions (%)", "Prevalence of HI (%)", "Fisheries (%)", "Gunshot (%)", "Boat (%)", "Other (%)", "OR (%)", "WA (%)")
...
summary(lm(value ~ Year.of.Observation*variable, data = allprev.type.stats))
