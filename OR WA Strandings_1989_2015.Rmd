...
age_model <- glm(cnt ~ Age.Class, family = poisson(link = log), data = age_sex_stats)
summary(age_model)
...
TukeyHSD(aov(cnt ~ Pinniped.Common.Name, data = species_year))
...
TukeyHSD(aov(cnt ~ Month.of.Observation + Age.Class, data = monthly_age_stats))
...
colnames(species_props) <- c("Species", "Total (n)", "Male (%)", "Female (%)", "Pup (%)", "Yearling (%)", "Subadult (%)", "Adult (%)", "Unidentified Age (%)")
