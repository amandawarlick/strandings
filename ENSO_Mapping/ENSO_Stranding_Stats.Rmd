...
corrplot(cor_test_small$r, type = "lower", order = "original", p.mat = cor_test_small$P, 
         insig = "blank", sig.level = 0.01, tl.col = "black", tl.cex = .7, number.cex = .2)
corrplot(cor_test_0vs1$r, type = "lower", order = "original", p.mat = cor_test_0vs1$P, 
         insig = "blank", sig.level = 0.01, tl.col = "black", tl.cex = .7, number.cex = .7)
#SST and SST_anom_lag2s are correlated
corrplot(cor_test_0vs2$r, type = "lower", order = "original", p.mat = cor_test_0vs2$P, 
         insig = "blank", sig.level = 0.01, tl.col = "black", tl.cex = .7, number.cex = .7)
corrplot(cor_test_0vs3$r, type = "lower", order = "original", p.mat = cor_test_0vs3$P, 
         insig = "blank", sig.level = 0.01, tl.col = "black", tl.cex = .7, number.cex = .7)
...
## Plot the results
plot(annual_anom_comb$Year.of.Observation, annual_anom_comb$cnt_all, main="Eruptions of Old Faithful")
# restricted model
abline(r.reg, col = "red",lwd = 2, lty = "dashed")
# unrestricted model 1
segments(0, ur.reg2$coefficients[1], 2003,
ur.reg2$coefficients[1]+2003*ur.reg2$coefficients[2], col= 'blue')
# unrestricted model 2
segments(2003, ur.reg1$coefficients[1]+2003*ur.reg1$coefficients[2],
2016, ur.reg1$coefficients[1]+2016*ur.reg1$coefficients[2], col= 'blue')
...
mod_select <- regsubsets(cnt_all ~ 
      SST_39_anom + SST_44 + CBA_South_lag1 + SST_39_lag1 + Wind_39_lag1 + Upwelling_39_lag1 + PDO_lag1 + Wind_39_lag2 + Upwelling_39_lag2 +  SST_39_lag3 + Period, data = monthly_anom_comb, nbest = 5)
summary(mod_select)

#best model without lag data was: SST_44, SST_39_anom, Upwelling_39, Wind_39
nb <- summary(glm.nb(cnt_all ~ SST_39_anom + SST_44 + Upwelling_39 + CBA_South + Wind_39, data = monthly_anom_comb))
...
summary(glm.nb(cnt_all ~ MEI + Wind_39 + CBA_North + MEI_lag1 + CBA_North_lag2 + MEI_lag3 + Upwelling_45_lag3 + CBA_North_lag3, data = monthly_anom %>% filter(Pinniped.Common.Name == 'Steller sea lion')))

summary(glm(cnt_all ~ MEI + Wind_39 + CBA_North + MEI_lag1 + CBA_North_lag2 + MEI_lag3 + Upwelling_45_lag3 + CBA_North_lag3 + Period, data = monthly_anom %>% filter(Pinniped.Common.Name == 'Steller sea lion')), family = 'quasipoisson') 
...
qp <- summary(glm(cnt_HI ~ MEI + SST_39 + Wind_44 + CBA_South + SST_39_lag1 + Wind_39_lag3 + Period, 
          data = monthly_anom_comb, family = 'quasipoisson'))
...
nb <- summary(glm.nb(cnt_HI ~ PDO + SST_39 + Wind_44 + CBA_South +
          Upwelling_45_lag2 + CBA_South_lag2,
          data = monthly_anom %>% filter(Pinniped.Common.Name == 'California sea lion')))

qp <- summary(glm(cnt_HI ~ PDO + SST_39 + Wind_44 + CBA_South +
          Upwelling_45_lag2 + CBA_South_lag2, 
          data = monthly_anom %>% filter(Pinniped.Common.Name == 'California sea lion'), family = 'quasipoisson'))
pchisq(deviance(nb), df.residual(nb), lower.tail = F) # pvalue = 0.99, which means model is an okay fit?
...
#           MEI_lag1 + PDO_lag1 + NPGO_lag1 + SST_39_lag1 + SST_44_lag1 + Wind_39_lag1 + Wind_44_lag1 + Upwelling_39_lag1 + Upwelling_45_lag1 + CBA_North_lag1 + CBA_South_lag1 + SST_39_anom_lag1 + SST_44_anom_lag1 +
#           MEI_lag2 + PDO_lag2 + NPGO_lag2 + SST_39_lag2 + SST_44_lag2 + Wind_39_lag2 + Wind_44_lag2 + Upwelling_39_lag2 + Upwelling_45_lag2 + CBA_North_lag2 + CBA_South_lag2 + SST_39_anom_lag2 + SST_44_anom_lag2 +
#           MEI_lag3 + PDO_lag3 + NPGO_lag3 + SST_39_lag3 + SST_44_lag3 + Wind_39_lag3 + Wind_44_lag3 + Upwelling_39_lag3 + Upwelling_45_lag3 + CBA_North_lag3 + CBA_South_lag3 + SST_39_anom_lag3 + SST_44_anom_lag3 + Period, data = monthly_anom %>% filter(Pinniped.Common.Name == 'Harbor seal'), nbest = 3, really.big = T)
# summary(mod_select)
...
#           MEI_lag1 + PDO_lag1 + NPGO_lag1 + SST_39_lag1 + SST_44_lag1 + Wind_39_lag1 + Wind_44_lag1 + Upwelling_39_lag1 + Upwelling_45_lag1 + CBA_North_lag1 + CBA_South_lag1 + SST_39_anom_lag1 + SST_44_anom_lag1 +
#           MEI_lag2 + PDO_lag2 + NPGO_lag2 + SST_39_lag2 + SST_44_lag2 + Wind_39_lag2 + Wind_44_lag2 + Upwelling_39_lag2 + Upwelling_45_lag2 + CBA_North_lag2 + CBA_South_lag2 + SST_39_anom_lag2 + SST_44_anom_lag2 +
#           MEI_lag3 + PDO_lag3 + NPGO_lag3 + SST_39_lag3 + SST_44_lag3 + Wind_39_lag3 + Wind_44_lag3 + Upwelling_39_lag3 + Upwelling_45_lag3 + CBA_North_lag3 + CBA_South_lag3 + SST_39_anom_lag3 + SST_44_anom_lag3 + Period, data = monthly_anom %>% filter(Pinniped.Common.Name == 'Steller sea lion'), nbest = 3, really.big = T)
# summary(mod_select)

summary(glm.nb(cnt_all ~ MEI + Wind_39 + CBA_North + MEI_lag1 + CBA_North_lag2 + MEI_lag3 + Upwelling_45_lag3 + CBA_North_lag3, data = monthly_anom %>% filter(Pinniped.Common.Name == 'Steller sea lion')))

summary(glm(cnt_all ~ MEI + Wind_39 + CBA_North + MEI_lag1 + CBA_North_lag2 + MEI_lag3 + Upwelling_45_lag3 + CBA_North_lag3, data = monthly_anom %>% filter(Pinniped.Common.Name == 'Steller sea lion')), family = 'quasipoisson')
...
summary(glm.nb(strand_anom_all + 10 ~ Wind_44 + CBA_South + SST_39_anom_lag1 +
          SST_44_lag1 + SST_39_lag2 + Wind_39_lag2 + Upwelling_39_lag2 + Wind_39_lag3 + Period, data = monthly_anom_comb))
