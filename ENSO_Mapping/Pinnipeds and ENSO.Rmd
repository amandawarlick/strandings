...
#Derive SST anomalies from temp values
meanSST <- ocean %>%
  select(matches("SST"), Month.of.Observation, -matches("_lag")) %>%
  group_by(Month.of.Observation) %>%
  summarize(SST_39_mean = mean(SST_39, na.rm = T),
...
#***Annual anomalies dataframe; separate species
...
annual_anom_plot <- ggplot(annual_anom_melt %>% filter(variable != 'cnt'), aes(Year.of.Observation, value, col = variable)) +
  geom_line() +
...
ocean_plot_data <- ocean %>%
  select(-matches("lag")) %>%
  select(-matches("ENSO")) %>%
  select(-c(Year.of.Observation, Month.of.Observation)) %>%
  melt(id.vars = 'Date')
...
ocean_plot_big <- ggplot(data = ocean_plot_data %>% 
                           filter(variable != 'MEI' & variable != 'PDO' & variable != 'NPGO' & !grepl("pwelling", variable)),
                     aes(Date, value, group = variable, col = variable)) +
  geom_line() +
  geom_line(aes(y = 0), col = 'grey60') +
  xlab("") + ylab("Monthly Oceanographic Index Anomalies") +
