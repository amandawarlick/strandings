...
pinnipeds_data_all <- read.csv("pinnipeds_data_all.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>%
  merge(MEI, by = c('Year.of.Observation', 'Month.of.Observation'), all = T)

pinnipeds_data_all$Month.of.Observation <- factor(pinnipeds_data_all$Month.of.Observation, levels = c('JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'))
...
pinnipeds_small <- pinnipeds_data_all %>%
  filter(Pinniped.Common.Name == 'Harbor seal' | Pinniped.Common.Name == 'California sea lion' |
           Pinniped.Common.Name == 'Steller sea lion') %>%
  select(-c(Affiliation, Common.Name, Genus, Species, Country, Restrand, Gear.Collection.Flag, Gear.Disposition.Flag, Other.Findings.upon.Level.Y.N, Other.Findings...Illness.Flag, Other.Findings...Injury.Flag, Other.Findings...Pregnant.Flag, How.Observed))
