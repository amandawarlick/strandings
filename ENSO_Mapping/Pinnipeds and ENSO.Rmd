...
#MEI, NPGO, PDO only - MEI and PDO covariance
...
ocean_plot_big <- ggplot(data = ocean_plot_data %>% 
                           filter(variable != 'MEI' & variable != 'PDO' & variable != 'NPGO' & !grepl("pwelling", variable) & variable != 'SST_39' & variable != 'SST_44'),
                     aes(Date, value, group = variable, col = variable)) +
  geom_line() +
  geom_line(aes(y = 0), col = 'grey60') +
  xlab("") + ylab("Monthly Oceanographic Index Anomalies") +
...
#Upwelling buys mirror each other, but 39N is much higher in value, 45N is more negative
...
geom_polygon(aes(x = long, y = lat, group = group), fill = "grey93", color = "grey50", size = 0.2) + 
  ...
  geom_density2d(data = pinnipeds_data_mapping, aes(x = long_corr, y = lat_corr), bins = 4, col = 'red') +
...
testanimate <- ggplot(data = all_yr_data, aes(x = Year.of.Observation, y = cnt, frame = Year.of.Observation, cumulative = T)) + 
...
HI_type_animate <- ggplot(pnw_state_outline) +
geom_polygon(aes(x = long, y = lat, group = group), fill = "grey93", color = "grey50", size = 0.2) +
  coord_fixed(1, ylim = c(42.3, 49.15), xlim = c(-125, -121.8)) +
  # geom_point(data = pinnipeds_data_mapping_HI, aes(x = long_corr, y = lat_corr), color = 'grey20', size = 0.2, alpha = 0.8)  +
  geom_density2d(data = pinnipeds_data_mapping_HI, aes(x = long_corr, y = lat_corr, frame = Month.of.Observation, cumulative = F), bins = 4, col = 'red') +
  scale_alpha(range = c(0, 0.1), guide = FALSE) +
  xlab("") + ylab("") +
  #facet_wrap(~ Interaction.Type) +
  #theme_nothing() +
...
gganimate(testanimate, interval = 1.0, filename = "~/Documents/R/Strandings/ENSO_Mapping/HImonthmap.gif")
