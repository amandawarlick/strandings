...
colnames(age_sex_table) <- c("Age Class", "Female (N)", "Female (%)", "Male (N)", "Male (%)", "Unid. (N)", "Unid. (%)")
...
colnames(bigtable_WA_HI_table) <- c("County", "All Strandings (%)", "Human Interactions (%)", "Fisheries (%)", "Gunshot (%)", "Boat (%)", "Other (%)")
...
bigtable_OR_HI_table <- bigtable_OR_HI_type %>%
  ...
  select(County, all.prop, HI.prop, Prop.fish, Prop.shot, Prop.boat, Prop.other)
colnames(bigtable_OR_HI_table) <- c("County", "All Strandings (%)", "Human Interactions (%)", "Fisheries (%)", "Gunshot (%)", "Boat (%)", "Other (%)")
...
colnames(Sex_HI_type_table) <- c("Class", "All Strandings (%)", "Prevalence of HI (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")

sex_composition_table <- write.csv(Sex_HI_type_table, file = "./Figures/Sex_composition_table.csv", row.names = FALSE)
...
colnames(Age_HI_type_table) <- c("Class", "All Strandings (%)", "Prevalence of HI (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")

age_composition_table <- write.csv(Age_HI_type_table, file = "./Figures/age_composition_table.csv", row.names = FALSE)
...
species_age_sex_composition <- write.csv(species_props, file = "./Figures/species_age_sex_composition.csv", row.names = FALSE)
...
state_species_prop <- state_species_prop %>%
  ...
  select(Pinniped.Common.Name, prop.or, prop.wa_in, prop.wa_coast)
...
species_prop_HI <- species_prop_HI %>%
  ...
  select(Pinniped.Common.Name, cnt, all.prop, prop.or, prop.wa_coast, prop.wa_in, Prev.HI, Prop.Fisheries, Prop.Gunshot, Prop.Boat, Prop.Other) 
colnames(species_prop_HI) <- c("Species", "Total (n)", "All Strandings (%)", "OR (%)", "WA Coast (%)", "WA Inland (%)", "Prevalence of HI (%)", "Fisheries (%)", "Gunshot (%)", "Boat (%)", "Other (%)")
...
#   geom_point(data = pinnipeds_data.mapping, aes(x = Longitude, y = Latitude), size = 0.7)  +
#   stat_density2d(data = pinnipeds_data.mapping, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), size = 0.01, bins = 100, geom = "polygon") +
#   scale_fill_gradient(low = "yellow", high = "red", guide = FALSE) +
#   scale_alpha(range = c(0, 0.1), guide = FALSE) +
#   xlab("") + ylab("") +
#   theme(
