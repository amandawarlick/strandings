...
#region - all
sex_region <- pinnipeds_data %>%
  filter(Water.Body != 'NA') %>%
  group_by(Sex, Water.Body) %>%
  summarize(reg.cnt = n_distinct(National.Database.Number)) %>%
  dcast(Sex ~ Water.Body, value.var = 'reg.cnt') %>%
  transform(OR_Coast_p = round(OR_Coast/(Inland_WA+OR_Coast+WA_Coast)*100, 1),
            WA_Coast_p = round(WA_Coast/(Inland_WA+OR_Coast+WA_Coast)*100, 1),
            Inland_WA_p = round(Inland_WA/(Inland_WA+OR_Coast+WA_Coast)*100, 1)) %>%
  dplyr::select(Sex, OR_Coast_p, WA_Coast_p, Inland_WA_p)
...
Sex_HI_type_table <- pinnipeds_data %>%
  ...
  dplyr::select(c(Sex, all.cnt, All.prop, OR_Coast_p, WA_Coast_p, Inland_WA_p, HI.cnt, HI.prop, Prop.fish, Prop.shot, Prop.boat, Prop.other))
colnames(Sex_HI_type_table) <- c("Class", "All Strandings (n)", "All Strandings (%)", "OR Coast (%)", "WA Coast (%)", "WA Inland (%)", "HI Cases (n)", "HI Prevalence (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")
...
age_region <- pinnipeds_data %>%
  filter(Water.Body != 'NA') %>%
  group_by(Age.Class, Water.Body) %>%
  summarize(reg.cnt = n_distinct(National.Database.Number)) %>%
  dcast(Age.Class ~ Water.Body, value.var = 'reg.cnt') %>%
  transform(OR_Coast_p = round(OR_Coast/(Inland_WA+OR_Coast+WA_Coast)*100, 1),
            WA_Coast_p = round(WA_Coast/(Inland_WA+OR_Coast+WA_Coast)*100, 1),
            Inland_WA_p = round(Inland_WA/(Inland_WA+OR_Coast+WA_Coast)*100, 1)) %>%
  dplyr::select(Age.Class, OR_Coast_p, WA_Coast_p, Inland_WA_p)
...
Age_HI_type_table <- pinnipeds_data %>%
  ...
  dplyr::select(c(Age.Class, all.cnt, All.prop, OR_Coast_p, WA_Coast_p, Inland_WA_p, HI.cnt, Prev.HI, Prop.fish, Prop.shot, Prop.boat, Prop.other))
Age_HI_type_table <- Age_HI_type_table[order(Age_HI_type_table$Age),]
colnames(Age_HI_type_table) <- c("Class", "All Strandings (n)", "All Strandings (%)", "OR Coast (%)", "WA Coast (%)", "WA Inland (%)", "HI Cases (n)", "HI Prevalence (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")
...
species_props <- pinnipeds_data %>%
  ...
  dplyr::select(Pinniped.Common.Name, Tot, Prop.Male, Prop.Fem, Prop.Unid, Prop.pup, Prop.year, Prop.sub, Prop.ad, Prop.un)
colnames(species_props) <- c("Species", "All Strandings (n)", "Male (%)", "Female (%)", "Unid. Sex (%)", "Pup (%)", "Yearling (%)", "Subadult (%)", "Adult (%)", "Unid. Age (%)")
