...
age_all <- pinnipeds_data %>%
  transform(Age.Class = ifelse(Year.of.Observation < 2001, 'NA', as.character(Age.Class))) %>%
...
species_prop <- pinnipeds_data %>%
  ...
  #filter(Pinniped.Common.Name != 'Unidentified') %>%
  select(Pinniped.Common.Name, all.prop)
...
#   transform(Age.Class = ifelse(Year.of.Observation < 2001, 'NA', Age.Class)) %>%
#   filter(Pinniped.Common.Name != 'Unidentified') %>%
...
formerge <- species_sex_props %>% 
  ...
  transform(Prop.Fem = round(Female/(Female+Male+Unid), 3)*100,
            Prop.Male = round(Male/(Female+Male+Unid), 3)*100,
            Prop.Unid = round(Unid/(Female+Male+Unid), 3)*100) %>%
  dplyr::select(Pinniped.Common.Name, Prop.Fem, Prop.Male, Prop.Unid)
...
species_props <- pinnipeds_data %>%
  ...
  merge(species_prop, by = 'Pinniped.Common.Name') %>%
  dplyr::select(Pinniped.Common.Name, Tot, all.prop, Prop.Male, Prop.Fem, Prop.Unid, Prop.pup, Prop.year, Prop.sub, Prop.ad, Prop.un)
...
species_prop_reg <- pinnipeds_data %>%
...
species_HI_prev <- species_HI_prev %>%
    transform(Prev.HI = round(Y/(Y+N+CBD), 3)*100) %>%
  rename(HI.cnt = Y) %>%
  dplyr::select(Pinniped.Common.Name, HI.cnt, Prev.HI) %>%
...
species_prop_HI <- species_prop_HI %>%
  ...
  merge(species_prop_reg, by = 'Pinniped.Common.Name') %>%
  dplyr::select(Pinniped.Common.Name, cnt, all.prop, prop.or, prop.wa_coast, prop.wa_in, HI.cnt, Prev.HI, Prop.Fisheries, Prop.Gunshot, Prop.Boat, Prop.Other) 
...
#by region
region <- pinnipeds_data %>%
  filter(Water.Body != 'NA') %>% group_by(Water.Body) %>%
  summarize(cnt = n_distinct(National.Database.Number))
HI <- pinnipeds_data %>%
  filter(Findings.of.Human.Interaction == 'Y') %>%
  summarize(cnt = round((n_distinct(National.Database.Number)/14939*100), 1))
HI_type_p <- pinnipeds_data %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
...
```{r datacleaningexploring, echo = F, eval = F}

#bag of words experimenting

HIOtherID_all <- pinnipeds_data %>%
  filter(Interaction.Type == 'Other') %>%
  #filter(!is.na(Other.Human.Int.Description)) %>%
  filter(Other.Human.Int.Description != 'NA') %>%
  select(National.Database.Number, Pinniped.Common.Name, Year.of.Observation, Affiliation, State, County, Other.Human.Int.Description) %>%
  transform(Other.Cat = ifelse(grepl("dog", Other.Human.Int.Description) | 
                                 grepl("Dog", Other.Human.Int.Description) |
                                 grepl("canine", Other.Human.Int.Description), 'Dog', 
                               ifelse(grepl("arass", Other.Human.Int.Description) |
                                   grepl("arrass", Other.Human.Int.Description) |
                                   grepl("eople", Other.Human.Int.Description) |
                                   grepl("ublic", Other.Human.Int.Description) |
                                   grepl("itizen", Other.Human.Int.Description), 'Harassment', 
                                 ifelse(grepl("oil ", Other.Human.Int.Description), 'Oil Spill',
                                 ifelse(grepl("pick up", Other.Human.Int.Description) |
                                          grepl("llegal", Other.Human.Int.Description) |
                                          grepl("picked up", Other.Human.Int.Description), 'Illegal pick-up', 
                                        ifelse(grepl("decapita", Other.Human.Int.Description) | 
                                                 grepl("Decapit", Other.Human.Int.Description) |
                                                 grepl("head removed", Other.Human.Int.Description) |
                                                 grepl("behead", Other.Human.Int.Description), 'Decapitated',
                                               ifelse(grepl("debris", Other.Human.Int.Description) |
                                                        grepl("entangle", Other.Human.Int.Description) |
                                                        grepl("around neck", Other.Human.Int.Description) |
                                                        grepl("rope", Other.Human.Int.Description), 'Marine debris',
                                          'Other'))))))) %>%
  group_by(Other.Cat) %>%
  summarize(cnt = n_distinct(National.Database.Number))
