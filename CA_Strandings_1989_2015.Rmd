...
pinnipeds_data_ca_recent <- read.csv("pinnipeds_data_ca.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>%
  dplyr::select(-matches("Flag")) %>%
  dplyr::select(-Restrand) %>%
  dplyr::select(-matches("Hazardous"))

pinnipeds_data_ca_early <- read.csv("CA_data1982_2005.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>%
  mutate(Date.of.Occurrence = mdy(Date.of.Occurance)) %>%
  dplyr::select(-Date.of.Occurance) %>%
  transform(Month.of.Observation = toupper(month(Date.of.Occurrence, label = T, abbr = T)),
            Year.of.Observation = year(Date.of.Occurrence),
            Day.of.Observation = day(as.Date(Date.of.Occurrence))) %>%
  transform(Pinniped.Common.Name = case(Common.Name.Earlyyrs)) %>%
  transform(Sex = ifelse(Sex == 'M', 'Male', ifelse(Sex == 'F', 'Female', 'Unid'))) %>%
  transform(Age.Class = case(Age.Class)) %>%
  transform(Age.Class = ifelse(Age.Class == 'Unknown', 'Unid', Age.Class)) %>%
  transform(Genus = case(Genus)) %>%
  transform(Species = tolower(Species)) %>%
  transform(County = case(County)) %>%
  transform(City = case(City)) %>%
  dplyr::rename(Observation.Status = Initial.Condition) %>%
  transform(Evidence.of.Human.Interaction = 
              ifelse(grepl("FISHERY", Type.of.Occurance) | grepl("GILLNET", Type.of.Occurance) | 
                       grepl("COLLISION", Type.of.Occurance) | grepl("HUMAN", Type.of.Occurance) |
                       grepl("MARINE DEBRIS", Type.of.Occurance) | Type.of.Occurance %in% c("ENTRAINMENT", "SHOOTING-RELATED", "GAFFING-RELATED", "CLUBBING-RELATED", "EXPLOSION-RELATED", "HARPOONING-RELATED", "ILLEGAL CAPTURE", "OIL/TAR SPILL-RELATED", "POACHING-RELATED"), 'Y', 'N')) %>%
  transform(Interaction.Type = ifelse(Evidence.of.Human.Interaction == 'N', 'NA', 
              ifelse(grepl("FISHERY", Type.of.Occurance) | grepl("GILLNET", Type.of.Occurance), 'Fisheries',
                  ifelse(Type.of.Occurance == 'BOAT COLLISION', 'Boat', 
                         ifelse(Type.of.Occurance == 'SHOOTING-RELATED', 'Gunshot', 'Other'))))) %>%
  dplyr::select(-Common.Name.Earlyyrs) %>%
  transform(Mammal.Type = 
        ifelse(Genus %in% c('Phoca', 'Zalophus', 'Mirounga', 'Arctocephalus', 'Callorhinus') | 
                 grepl('sea lion', Pinniped.Common.Name) | grepl('pinniped', Pinniped.Common.Name) |
                 grepl('phocid', Pinniped.Common.Name) | grepl('otariid', Pinniped.Common.Name) |
                 grepl('fur seal', Pinniped.Common.Name), 'Pinniped', 
               ifelse(grepl("sea turtle", Pinniped.Common.Name) | grepl("shark", Pinniped.Common.Name), 'Other', 'Cetacean'))) 

pinnipeds_data_ca <- bind_rows(pinnipeds_data_ca_early, pinnipeds_data_ca_recent)



# typecleanup <- pinnipeds_data_ca_early %>%
#   group_by(Mammal.Type, Pinniped.Common.Name) %>%
#   summarize(n_distinct(National.Database.Number))

pinnipeds_data_ca$Age.Class <- factor(pinnipeds_data_ca$Age.Class, 
          levels = c("Pup", "Yearling", "Subadult", "Adult", "Unid"))

pinnipeds_data_ca$Month.of.Observation <- factor(pinnipeds_data_ca$Month.of.Observation, 
...
ggsave(Sex_all_yr_figure_stack, file = "./Figures_CA/Sex_all_yr_figure_stack.pdf")
...
Sex_all_yr_figure <- 
  ...
  plot_theme(legend.position = 'top')
ggsave(Sex_all_yr_figure, file = "./Figures_CA/Sex_all_yr_figure.pdf")
...
Sex_all_yr_boxplot <- 
  ...
  plot_theme(legend.position = 'top')
ggsave(Sex_all_yr_boxplot, file = "./Figures_CA/Sex_all_yr_boxplot.pdf")
...
Sex_all_month_figure <- 
  ...
  xlab("") + ylab("Proportion of Strandings") +
  plot_theme(legend.position = 'top')
ggsave(Sex_all_month_figure, file = "./Figures_CA/Sex_all_mo_figure.pdf")
...
HI_type_sex_figure <- ggplot(Sex_HI_yr, aes(x = Interaction.Type, y = cnt, fill = Sex)) +
  ...
  xlab("") + ylab("Proportion of Strandings") +
  plot_theme(legend.position = 'top')
ggsave(HI_type_sex_figure, file = "./Figures_CA/HI_type_sex_figure.pdf")
...
ggsave(age_sex_all_figure, file = "./Figures_CA/age_sex_all_figure.pdf")
...
colnames(age_sex_table) <- c("Age", "Female (N)", "Female (%)", "Male (N)", "Male (%)", "Unid. (N)", "Unid. (%)")
average_sex <- c(as.character("Average"), "--", mean(age_sex_table[,3]), "--", mean(age_sex_table[,5]), "--", mean(age_sex_table[,7]))

age_sex_table <- rbind(age_sex_table, average_sex, stringsAsFactors = F)


age_sex_table <- write.csv(age_sex_table, file = "./Figures_CA/age_sex_table.csv", row.names = F)


age_sex_stats <- pinnipeds_data_ca %>%
  filter(Sex != 'Unid') %>%
  group_by(Sex, Year.of.Observation, Age.Class) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 

#indicates that all are different from each other
age_model <- glm(cnt ~ Age.Class*Sex, family = poisson(link = log), data = age_sex_stats)
summary(age_model)
# summary(aov(cnt ~ Sex*Age.Class, data = age_sex_stats))
...
data_by_year <- pinnipeds_data_ca %>% 
  group_by(Year.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 

#Statistically significant change in all stranding cases over time (p < 0.05, 25 strandings per year increase).
...
ggsave(all_yr_figure, file = "./Figures_CA/all_year_figure.pdf")
...
HI_year_figure <- 
  ...
  plot_theme(legend.position = 'top') +
  scale_color_brewer(palette = 'Set1')
ggsave(HI_year_figure, file = "./Figures_CA/HI_year_figure.pdf")
```

```{r HI types over time}
#Are specific types of HI cases changing over time?

HI_by_year_type <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
...
HI_type_year_figure <- 
  ...
  plot_theme(legend.position = 'top') + 
  scale_color_brewer(palette = 'Set1')
ggsave(HI_type_year_figure, file = "./Figures_CA/HI_type_year_figure.pdf")
...
species_figure <- species_year %>%
  ...
  xlab("") + ylab("Annual Strandings") +
  plot_theme()
ggsave(species_figure, file = "./Figures_CA/species_figure.pdf")
...
##HI cases
species_year_HI <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
  group_by(Year.of.Observation, Pinniped.Common.Name) %>%
  summarize(cnt = n_distinct(National.Database.Number))
...
species_year_HI_figure <- 
  ...
  scale_y_continuous(labels = comma) +
  plot_theme()
ggsave(species_year_HI_figure, file = "./Figures_CA/species_year_HI_figure.pdf")
...
species_year_FI <- pinnipeds_data_ca %>%
...
species_fish_figure <- 
  ...
  plot_theme()
ggsave(species_fish_figure, file = "./Figures_CA/species_fish_figure.pdf")
...
species_year_shot <- pinnipeds_data_ca %>%
...
species_shot_figure <- 
  ...
  plot_theme()
ggsave(species_shot_figure, file = "./Figures_CA/species_shot_figure.pdf")
...
ggsave(species_all_figure, file = "./Figures_CA/species_all_figure.pdf")
...
ggsave(species_all_figure_stacked, file = "./Figures_CA/species_all_figure_stacked.pdf")
...
species_HI_figure <- 
  ...
  plot_theme(legend.position = 'top') + 
  scale_color_brewer(palette = 'Set1')
ggsave(species_HI_figure, file = "./Figures_CA/species_HI_figure.pdf")
...
species_year_FI_figure <- 
  ...
  plot_theme(legend.position = 'top') + 
  scale_color_brewer(palette = 'Set1')
ggsave(species_year_FI_figure, file = "./Figures_CA/species_year_FI_figure.pdf")
...
stacked_FI_figure <- 
  ...
  plot_theme(legend.position = 'top') + 
  scale_fill_brewer(palette = 'Set2') 
ggsave(stacked_FI_figure, file = "./Figures_CA/stacked_FI_figure.pdf")

#Exploring single species 

# year.species.FI.TEST <- pinnipeds_data_ca %>%
...
year_prop_HI <- pinnipeds_data_ca %>%
...
HI_prev_species_TEST <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>% 
...
HI_prev_species_table <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>% 
...
year_prop_FI <- pinnipeds_data_ca %>%
...
HI_species_prev <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
  group_by(Year.of.Observation, 
...
ggplot(aes(x = factor(Month.of.Observation), y = sum, fill = Sex)) +
  ...
  plot_theme(legend.position = 'top') + 
  scale_fill_brewer(palette = 'Set1')
ggsave(monthly_sex_figure, file = "./Figures_CA/monthly_sex_figure.pdf")
...
ggsave(monthly_age_figure, file = "./Figures_CA/monthly_age_figure.pdf")
...
age_HI_stats <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y' & 
...
monthly_age <- pinnipeds_data_ca %>% group_by(Month.of.Observation, Age.Class) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
...
monthly_age_HI <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
  group_by(Month.of.Observation, Age.Class) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  dcast(Month.of.Observation ~ Age.Class, value.var = 'cnt') %>%
...
monthly_years_figure <- 
  ...
  plot_theme() #+ 
  #scale_color_brewer()
ggsave(monthly_years_figure, file = "./Figures_CA/monthly_years_figure.pdf")

#Are there seasonal patterns in HI cases?

monthly_HI <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y') %>%
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
  group_by(Month.of.Observation, Year.of.Observation, Interaction.Type, Sex, Age.Class) %>%
  summarize(cnt = n_distinct(National.Database.Number))

monthly_HI_yr_mo <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y') %>%
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
...
monthly_HI_YNCBD <- pinnipeds_data_ca %>% 
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
  group_by(Findings.of.Human.Interaction, Month.of.Observation) %>% 
  summarize(cnt = n_distinct(National.Database.Number)) 

monthly_HI_YNCBD_prop <- pinnipeds_data_ca %>% 
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
  group_by(Findings.of.Human.Interaction, Month.of.Observation) %>% 
  summarize(cnt = n_distinct(National.Database.Number)) %>%
...
monthly_HI_figure <- monthly_HI %>%
  ...
  plot_theme() + 
  scale_fill_brewer(palette = 'Set1')
ggsave(monthly_HI_figure, file = "./Figures_CA/monthly_HI_figure.pdf")
...
monthly_HI_figure_stacked <- monthly_HI %>% 
  ...
  plot_theme(legend.position = 'top') + 
  scale_fill_brewer(palette = 'Set1')
ggsave(monthly_HI_figure_stacked, file = "./Figures_CA/monthly_HI_figure_stacked.pdf")
...
ggplot(aes(x = factor(Month.of.Observation), y = sum, fill = Age.Class)) +
  ...
  plot_theme(legend.position = 'top') + 
  scale_fill_brewer(palette = 'Set1')
ggsave(monthly_HI_age_figure, file = "./Figures_CA/monthly_HI_age_figure.pdf")
...
ggsave(monthly_HI_year_figure, file = "./Figures_CA/monthly_HI_year_figure.pdf")

#Is there an effect of month on FI?
monthly_FI <- pinnipeds_data_ca %>%
  filter(Fishery.Interaction == 'Y') %>%
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
  group_by(Year.of.Observation, Month.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number))
...
monthly_FI_year_figure <- 
  ...
  plot_theme() #+ 
 # scale_color_brewer(palette = 'Set3')
ggsave(monthly_FI_year_figure, file = "./Figures_CA/monthly_FI_year_figure.pdf")
```
...
monthly_species <- pinnipeds_data_ca %>%
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
...
monthly_species_figure <- 
  ...
  plot_theme(legend.position = 'top') + 
  scale_color_brewer(palette = 'Set3')
ggsave(monthly_species_figure, file = "./Figures_CA/monthly_species_figure.pdf")
...
monthly_species_HI <- pinnipeds_data_ca %>%
  filter(Observation.Status == 'ALIVE' | Observation.Status == 'FRESH DEAD') %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
  filter(Findings.of.Human.Interaction == 'Y') %>%
  group_by(Month.of.Observation, Pinniped.Common.Name, Year.of.Observation) %>%
...
monthly_species_HI_figure <- 
  ...
  plot_theme(legend.position = 'top') + 
  scale_color_brewer(palette = 'Set3')
ggsave(monthly_species_HI_figure, file = "./Figures_CA/monthly_species_HI_figure.pdf")

#Human interactions monthly each year

monthly_HI_year <- pinnipeds_data_ca %>% 
  filter(Pinniped.Common.Name != 'Unidentified') %>%
  filter(Findings.of.Human.Interaction == 'Y') %>%
  group_by(Month.of.Observation, Year.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 
...
monthly_species_FI_shot <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
...
monthly_FI_shot_year_figure <- monthly_species_FI_shot %>%
  ...
  plot_theme(legend.position = 'top') + 
  scale_color_brewer(palette = 'Set3')
ggsave(monthly_FI_shot_year_figure, file = "./Figures_CA/monthly_FI_shot_year_figure.pdf")

```


```{r CA counties}
#all strandings by county - CA

allCA_bycounty <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & County != 'Unknown' & State == 'CA') %>% 
  group_by(County, Year.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 

allCA_bycounty_figure <- ggplot(allCA_bycounty, aes(x = County, y = cnt)) + 
  geom_bar(stat = 'identity')  + 
  ylab("Total Strandings") + xlab("") + 
  scale_y_continuous(labels = comma) +
  plot_theme() + 
  scale_fill_brewer(palette = 'Set1')
ggsave(allCA_bycounty_figure, file = "./Figures_CA/allCA_bycounty_figure.pdf")

#Some counties have higher/lower strandings compared to others
summary(aov(cnt ~ County, allCA_bycounty))

#Increasing in some counties, decreasing in others
CAcounty_model <- glm(cnt ~ County + Year.of.Observation, family = poisson(link = log), data = allCA_bycounty)

summary(CAcounty_model)
autoplot(CAcounty_model)

#all HI by county - CA

CAHI_bycounty <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & 
...
#summary(aov(cnt ~ County + Interaction.Type + Year.of.Observation, CAHI_bycounty))

#human interaction cases higher in grays, pacific, pierce, whatcom
CAcounty_HI_model <- glm(cnt ~ County, family = poisson(link = log), CAHI_bycounty)
summary(CAcounty_HI_model)
autoplot(CAcounty_HI_model)

#increasing in grays harbor, pacific, pierce, none decreasing
summary(glm(cnt ~ County + Year.of.Observation, family = poisson(link = log), data = CAHI_bycounty))
...
summary(glm(cnt ~ County*Interaction.Type*Year.of.Observation, family = poisson(link = log), CAHI_bycounty))
...
bigtable_CA_stats <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown') %>% 
  group_by(County) %>%
  summarize(all.cnt = n_distinct(National.Database.Number)) %>%
  transform(all.prop = round(all.cnt/sum(all.cnt), 2)*100)

bigtable_CA_HI_stats <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown' & 
           Findings.of.Human.Interaction == 'Y' & 
           Interaction.Type != 'NA') %>% 
  group_by(County) %>%
  ...
  merge(bigtable_CA_stats, by = 'County')

#Not quite right - shows which proportions are different from each other
pairwise.prop.test(bigtable_CA_HI_stats$HI.cnt, bigtable_CA_HI_stats$all.cnt)
...
prop.test(bigtable_CA_HI_stats$HI.cnt, bigtable_CA_HI_stats$all.cnt)

#So I think I need to test the proportions, because that would assume n is the baseline?
bigtable_CA_props <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown') %>% 
  group_by(County) %>%
  summarize(all.cnt = n_distinct(National.Database.Number)) %>%
  transform(all.prop = all.cnt/sum(all.cnt))

bigtable_CA_HI_props <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown' & 
           Findings.of.Human.Interaction == 'Y' & 
           Interaction.Type != 'NA') %>% 
  group_by(County) %>%
  ...
  merge(bigtable_CA_props, by = 'County') %>%
...
####
CAHI_bycounty_figure <- 
  ggplot(CAHI_bycounty, aes(x = County, y = cnt, fill = Interaction.Type)) + 
  geom_bar(stat = 'identity')  + 
  ylab("Total Human Interaction Cases") + xlab("") + 
  scale_y_continuous(labels = comma) +
  plot_theme(legend.position = 'top') + 
  scale_fill_brewer(palette = 'Set1') 
ggsave(CAHI_bycounty_figure, file = "./Figures_CA/CAHI_bycounty_figure.pdf")
...
#CA
bigtable_CA <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown') %>% 
  group_by(County) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  transform(all.prop = round(cnt/sum(cnt), 2)*100)

bigtable_CA_HI <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown' & 
           Findings.of.Human.Interaction == 'Y' & 
           Interaction.Type != 'NA') %>% 
  group_by(County) %>%
  ...
  merge(bigtable_CA, by = 'County')

bigtable_CA_HI_type <- pinnipeds_data_ca %>% 
  filter(County != 'NA' & State == 'CA' & County != 'Unknown' &
           Findings.of.Human.Interaction == 'Y' & 
           Interaction.Type != 'NA') %>% 
...
bigtable_CA_HI_type[is.na(bigtable_CA_HI_type)] <- 0

bigtable_CA_HI_table <- bigtable_CA_HI_type %>%
  ...
  merge(bigtable_CA_HI, by = 'County') %>%
  select(County, all.prop, HI.prop, Prop.fish, Prop.shot, Prop.boat, Prop.other)
colnames(bigtable_CA_HI_table) <- c("County", "All Strandings (%)", "Human Interactions (%)", "Fisheries (%)", "Gunshot (%)", "Boat (%)", "Other (%)")

CAcounties_table <- write.csv(bigtable_CA_HI_table, file = "./Figures_CA/CAcounties_table.csv", row.names = F)
...
# weightlength <- pinnipeds_data_ca %>% 
...
species_prop <- pinnipeds_data_ca %>%
  group_by(Pinniped.Common.Name) %>%
  summarize(cnt = n_distinct(National.Database.Number))
...
species_prop_table <- write.csv(species_prop, file = "./Figures_CA/species_prop_table.csv", row.names = FALSE)
                  

#Big proportion table for sex

Sex_table <- pinnipeds_data_ca %>%
  group_by(Sex) %>%
  summarize(all.cnt = n_distinct(National.Database.Number))
...
Sex_HI_table <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
  group_by(Sex) %>%
  summarize(HI.cnt = n_distinct(National.Database.Number)) %>%
...
Sex_HI_type_table <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
...
colnames(Sex_HI_type_table) <- c("Sex", "All Strandings (%)", "Prevalence of HI (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")

sex_composition_table <- write.csv(Sex_HI_type_table, file = "./Figures_CA/Sex_composition_table.csv", row.names = FALSE)


#Big table for age class

Age_table <- pinnipeds_data_ca %>%
...
#Adding in HI type and merging all three mini tables
Age_HI_type_table <- pinnipeds_data_ca %>%
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
...
colnames(Age_HI_type_table) <- c("Age", "All Strandings (%)", "Prevalence of HI (%)", "Fisheries Interactions (%)", "Gunshots (%)", "Boat Injuries (%)", "Other (%)")

Age_HI_type_table <- Age_HI_type_table[order(Age_HI_type_table$Age),]

age_composition_table <- write.csv(Age_HI_type_table, file = "./Figures_CA/age_composition_table.csv", row.names = FALSE)

#Species and age/sex composition
species_age_props <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
...
species_sex_props <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
...
species_props <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified') %>%
  group_by(Pinniped.Common.Name, Age.Class) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
...
species_age_sex_composition <- write.csv(species_props, file = "./Figures_CA/species_age_sex_composition.csv", row.names = FALSE)

########same but HI - looks very similar to all combined strandings
formerge_HI <- pinnipeds_data_ca %>%
...
species_props_HI <- pinnipeds_data_ca %>%
  filter(Pinniped.Common.Name != 'Unidentified' & Findings.of.Human.Interaction == 'Y') %>%
  group_by(Pinniped.Common.Name, Age.Class) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  dcast(Pinniped.Common.Name ~ Age.Class, value.var = 'cnt') %>%
...
# state_species_prop <- pinnipeds_data_ca %>%
...
# 
# species_bigtable <- pinnipeds_data_ca %>%
#   group_by(Pinniped.Common.Name) %>%
#   summarize(cnt = n_distinct(National.Database.Number)) %>%
#   transform(all.prop = round(cnt/sum(cnt), 2)*100)
# 
# species_HI_prev <- pinnipeds_data_ca %>%
#   #filter(Pinniped.Common.Name != 'Unidentified') %>% 
...
# 
# species_prop_HI <- pinnipeds_data_ca %>%
#   filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
...
# colnames(species_prop_HI) <- c("Species", "Total (n)", "All Strandings (%)", "OR (%)", "WA (%)", "Prevalence of HI (%)", "Fisheries (%)", "Gunshot (%)", "Boat (%)", "Other (%)")
# 
# species_HI_prop_table <- write.csv(species_prop_HI, file = "./Figures_CA/species_HI_prop_table.csv", row.names = F)

##Big Prevalence over time Table(s)
##all species combined, interaction types as a proportion of all cases, not just HI cases

numberperyear <- pinnipeds_data_ca %>% 
  group_by(Year.of.Observation) %>%
  summarize(yr.cnt = n_distinct(National.Database.Number))

allprev <- pinnipeds_data_ca %>%
  group_by(Findings.of.Human.Interaction, Year.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  dcast(Year.of.Observation ~ Findings.of.Human.Interaction, value.var = 'cnt') 
...
allprev_type <- pinnipeds_data_ca %>%
  group_by(Interaction.Type, Year.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
...
##individual species
numberperyearperspecies <- pinnipeds_data_ca %>% 
  group_by(Year.of.Observation, Pinniped.Common.Name) %>%
  summarize(yr.cnt = n_distinct(National.Database.Number))

allprev_species <- pinnipeds_data_ca %>%
...
allprev_type_species <- pinnipeds_data_ca %>%
...
# species_prop_HI_2 <- pinnipeds_data_ca %>%
#   filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
#   group_by(Pinniped.Common.Name, Interaction.Type) %>%
...
# latlong.cleaning <- pinnipeds_data_ca %>%
...
#CAbox <- c(-125, 41.91, -121.5, 46.28)

pinnipeds_data_ca_mapping <- pinnipeds_data_ca %>% 
  transform(Longitude = as.numeric(Longitude),
            Latitude = as.numeric(Latitude)) %>%
  filter(Longitude != 'NA')
...
# 
# ggmap(stamen) +
#    geom_point(data = pinnipeds_data_ca, aes(x = Longitude, y = Latitude)) +
...
#All CA - all strandings - black dots
allstrands <- 
  ggmap(get_map(location = allbox, source = "osm")) +
   geom_point(data = pinnipeds_data_ca_mapping, aes(x = Longitude, y = Latitude), size = .7, alpha = .5, color = 'grey50') +
scale_alpha(range = c(0, 0.3), guide = FALSE) +
  xlab("") + ylab("") +
  theme(
    axis.ticks = element_blank(),
    axis.text = element_blank())
ggsave(allstrands, file = "./Figures_CA/Maps/allstrands.pdf")

#CA - black dots
CAstrands <- 
  ggmap(get_map(location = CAbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping, aes(x = Longitude, y = Latitude), size = 0.7, alpha = .5, color = 'grey50') +
 scale_alpha(range = c(0, 0.3), guide = FALSE) +
  #xlab("") + ylab("") +
  theme(
    legend.key = element_blank(),
...
ggsave(CAstrands, file = "./Figures_CA/Maps/CAstrands.pdf")
...
allheatmap <- 
  ggmap(get_map(location = allbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping, aes(x = Longitude, y = Latitude), size = 0.7, alpha = .5, color = 'grey50')  +
  stat_density2d(data = pinnipeds_data_ca_mapping, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), size = 0.01, bins = 100, geom = "polygon") +
  ...
  theme(
    axis.ticks = element_blank(),
    axis.text = element_blank())
ggsave(allheatmap, file = "./Figures_CA/Maps/allheatmap.pdf")

#CA heatmap - all strandings
CAheatmap <- 
  ggmap(get_map(location = CAbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping, aes(x = Longitude, y = Latitude), size = 0.7, alpha = .5, color = 'grey50')  +
  stat_density2d(data = pinnipeds_data_ca_mapping, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), size = 0.01, bins = 100, geom = "polygon") +
  ...
  theme(
    ...
    #axis.text = element_blank(),
    panel.border = element_rect(colour = "black", fill = "NA", size = .5))
ggsave(CAheatmap, file = "./Figures_CA/Maps/CAheatmap.pdf")


#######HI Series
pinnipeds_data_ca_mapping_HI <- pinnipeds_data_ca_mapping %>% 
  filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA')

# pinnipeds_data_ca.mapping_HI_boat <- pinnipeds_data_ca.mapping %>% 
#   filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type == 'Boat')
# pinnipeds_data_ca.mapping_HI_fish <- pinnipeds_data_ca.mapping %>% 
#   filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type == 'Fisheries')
# pinnipeds_data_ca.mapping_HI_shot <- pinnipeds_data_ca.mapping %>% 
#   filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type == 'Gunshot')

#Human interactions - dots by HI type
HI_type_map <- 
  ggmap(get_map(location = allbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping_HI, aes(x = Longitude, y = Latitude, color = Interaction.Type), size = 0.7) +
#scale_alpha(range = c(0, 0.3), guide = FALSE) +
  xlab("") + ylab("") +
  plot_theme(
    legend.key = element_blank(),
    ...
    legend.position = 'top',
    panel.border = element_rect(colour = "black", fill = "NA", size = .5))
ggsave(HI_type_map, file = "./Figures_CA/Maps/HI_type_map.pdf")

#All HI - colors
HI_heatmap_colors <- 
ggmap(get_map(location = allbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping_HI, aes(x = Longitude, y = Latitude), size = 0.3, alpha = .5)  +
  geom_density2d(data = pinnipeds_data_ca_mapping_HI, aes(x = Longitude, y = Latitude, color = as.factor(Interaction.Type)), bins = 4, size = 1.5, alpha = .6) +
  # stat_density2d(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), size = 0.1, geom = "polygon") +
  scale_color_brewer(palette = 'Set2') +
  scale_alpha(range = c(0, 0.1), guide = FALSE) +
  xlab("") + ylab("") +
  theme(
...
ggsave(HI_heatmap_colors, file = "./Figures_CA/HI_heatmap_colors.pdf")

#All HI - facet
HI_heatmap_facet <-  
  ggmap(get_map(location = allbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping_HI, aes(x = Longitude, y = Latitude), color = 'grey50', size = 0.3, alpha = 0.5)  +
  geom_density2d(data = pinnipeds_data_ca_mapping_HI, aes(x = Longitude, y = Latitude), bins = 4, col = 'grey20') +
  scale_alpha(range = c(0, 0.1), guide = FALSE) +
  xlab("") + ylab("") +
  theme(
    axis.ticks = element_blank(),
...
ggsave(HI_heatmap_facet, file = "./Figures_CA/HI_heatmap_facet.pdf")
...
ggmap(get_map(location = CAbox, source = "osm")) +
  ...
  theme(
    ...
    strip.background = element_blank(),
    panel.border = element_rect(colour = "black", fill = "NA", size = 1)) 
ggsave(HI_heatmap_CA_colors, file = "./Figures_CA/HI_heatmap_CA_colors.pdf")
...
HI_heatmap_CA_facet <-  
  ...
  theme(
    ...
    #panel.border = element_rect(colour = "black", size = 1)) +
  facet_wrap(~ Interaction.Type)
ggsave(HI_heatmap_CA_facet, file = "./Figures_CA/HI_heatmap_CA_facet.pdf")


#######Species Series
pinnipeds_data_ca_mapping_sp <- pinnipeds_data_ca_mapping %>% filter(Pinniped.Common.Name != 'Unidentified') 

#colored by species - CA
CAheatmap_species <-   
  ggmap(get_map(location = CAbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude), size = 0.3, alpha = .5)  +
  geom_density2d(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude, color = as.factor(Pinniped.Common.Name)), bins = 4, size = 1.5, alpha = .6) +
  # stat_density2d(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), size = 0.1, geom = "polygon") +
  scale_color_brewer(palette = 'Set2') +
  scale_alpha(range = c(0, 0.1), guide = FALSE) +
  xlab("") + ylab("") +
...
#faceted species - CA
CAheatmap_species_facet <-   
  ggmap(get_map(location = CAbox, source = "osm")) +
  geom_point(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude), color = 'grey50', size = 0.3, alpha = 0.5)  +
  geom_density2d(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude), bins = 4, col = 'grey20') +
  # stat_density2d(data = pinnipeds_data_ca_mapping_sp, aes(x = Longitude, y = Latitude, fill = ..level.., alpha = ..level..), size = 0.1, geom = "polygon") +
  #scale_fill_gradient(low = "yellow", high = "red", guide = FALSE) +
  scale_alpha(range = c(0, 0.1), guide = FALSE) +
  xlab("") + ylab("") +
  theme(
    ...
    panel.border = element_rect(colour = "black", size = 1)) +
  facet_wrap(~ Pinniped.Common.Name)
ggsave(CAheatmap_species_facet, file = "./Figures_CA/CAheatmap_species_facet.pdf")

#Seasonal map exploration
pinniped_mapping_seasons <- pinnipeds_data_ca %>% 
  transform(Longitude = as.numeric(Longitude),
            Latitude = as.numeric(Latitude)) %>%
  filter(Longitude != 'NA') %>%
  transform(season = 
...
CAheatmap_CSL_seasons <-   
  ggmap(get_map(location = CAbox, source = "google", maptype = "roadmap")) +
