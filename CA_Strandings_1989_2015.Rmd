...
pinnipeds_data_ca_recent <- read.csv("pinnipeds_data_ca.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>%
  transform(Category = ifelse(grepl("SEAL", Common.Name) | grepl("SEA LION", Common.Name), "PINNIPED", "CETACEAN")) %>%
...
pinnipeds_data_ca_early <- read.csv("CA_data1982_2005.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>%
  dplyr::rename(Field.Number = Field.ID, National.Database.ID = Regional.ID, Affiliation = Examiner,
                Locality.Detail = Location.Details, Observation.Status = Initial.Condition) %>%
  dplyr::select(-matches("Tag")) %>%
  dplyr::select(-matches("Disposition")) %>%
  dplyr::select(-matches("Remarks")) %>%
