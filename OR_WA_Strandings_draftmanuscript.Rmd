...
species_year_mean <- pinnipeds_data %>%
  ...
  dplyr::summarize(cnt = n_distinct(National.Database.Number)) %>%
  group_by(Pinniped.Common.Name) %>%
  dplyr::summarize(mean = round(mean(cnt)))
...
HI_type_facet_map <- ggplot(pnw_state_outline) +
geom_polygon(aes(x = long, y = lat, group = group), fill = "grey", color = "black") + 
  ...
  geom_density2d(data = pinnipeds_data_mapping_HI, aes(x = long_corr, y = lat_corr), bins = 5, col = 'red') +
...
species_HI_facet_map <- ggplot(pnw_state_outline) +
geom_polygon(aes(x = long, y = lat, group = group), fill = "grey80", color = "grey40") + 
  coord_fixed(1, ylim = c(42.3, 49.15), xlim = c(-125, -121.8)) + 
  geom_point(data = pinnipeds_data_mapping_sp %>% filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA'), 
             aes(x = long_corr, y = lat_corr), col = 'grey20', size = 0.2, alpha = 0.5)  +
...
Human interaction cases - The overall number of HI cases increased significantly from 1989-2016 (z = 7.8, *p* < 0.001). An annual average of `r year_prop_HI %>% filter(Year.of.Observation < 2000) %>% dplyr::summarize(round(mean(Y), 0))` cases were documented throughout the 1990s and `r year_prop_HI %>% filter(Year.of.Observation > 1999) %>% dplyr::summarize(round(mean(Y), 0))` per year since 2000. Specifically, there was an increasing number of gunshot wounds (z = 4.6, *p* < 0.001), fisheries entanglements (z = 8.3, *p* < 0.001), and boat injuries (z = 3.3, *p* < 0.01) over the study period (`r figs("HI_timeseries", display = "cite")`b). The prevalence of HI cases has also increased overall (z = 15.5, *p* < 0.001) and specifically for gunshot wounds (z = 6.4, *p* < 0.001), fisheries entanglements (z = 5.3, *p* < 0.001), and boat injuries (z = 4.2, *p* < 0.001) (`r figs("HI_prev_final", display = "cite")`b). The prevalence of combined HI case types exceeded 20% in 2012, 2013, and 2015.
