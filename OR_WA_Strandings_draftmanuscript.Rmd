...
figs("countyfigure", caption = "Percent difference in the total number of human interaction cases in each county from the mean across all counties from 1989-2016. Points indicate locations of marine mammal stranding response networks.")
...
colnames(countymapdata) <- c("subregion", "all_cnt", "all_perc", "HI_cnt", "HI_perc", "Fish", "Gun", "Boat", "Other", "long", "lat", "group", "order", "region") 

countymapdata <- countymapdata %>%
  dplyr::select(-c(Fish, Gun, Boat, Other))

# center_perc_all <- pinnipeds_data %>%
#   group_by(Affiliation) %>%
#   summarize(cnt = n_distinct(National.Database.Number))
# center_perc_HI <- pinnipeds_data %>%
#   filter(Findings.of.Human.Interaction == 'Y' & Interaction.Type != 'NA') %>%
#   group_by(Affiliation) %>%
#   summarize(cnt_HI = n_distinct(National.Database.Number)) %>%
#   merge(center_perc_all, by = 'Affiliation', all = T) %>%
#   transform(perc_HI = round(cnt_HI/cnt, 2)*100) %>%
#   dplyr::rename(Center = Affiliation) %>%
#   merge(center_locations, by = 'Center', all = F)
...
# county_HI <- ggplot(countymapdata_melt_cnt %>% filter(variable == 'HI_cnt'), aes(x = long, y = lat, group = group)) +
# geom_polygon(aes(fill = value), size = 0.2) +
#   xlab("") + ylab("") +
#   coord_fixed(1) +
#   #expand_limits(x = countymapdata$long, y = countymapdata$lat) +
#   #coord_fixed(1, ylim = c(42.3, 49.15), xlim = c(-125, -120)) + 
...
#     strip.background = element_blank(),
#     panel.border = element_rect(colour = "black", fill = NA, size = 1)) + 
#   scale_fill_gradientn(colors = rainbow12, na.value = "white", name = "Human Interactions",
#               guide = guide_colorbar(direction = "horizontal",
#       barheight = unit(2, units = "mm"), barwidth = unit(40, units = "mm"),
#       draw.ulim = F, title.position = 'top', title.hjust = 0.5,
#       label.hjust = 0.5))
# 
# county_facet <- ggplot(countymapdata_melt_difs %>% filter(variable == 'HI_diff'), aes(x = long, y = lat, group = group)) +
# geom_polygon(aes(fill = value), size = 0.2) +
#   xlab("") + ylab("") +
#   coord_fixed(1) +
#   #expand_limits(x = countymapdata$long, y = countymapdata$lat) +
...
#     strip.background = element_blank(),
#     panel.border = element_rect(colour = "black", fill = NA, size = 1)) +
#   scale_fill_gradientn(colors = rainbow12, na.value = "white", name = "Cases",
#             guide = guide_colorbar(direction = "horizontal",
#       barheight = unit(2, units = "mm"), barwidth = unit(40, units = "mm"),
...
geom_polygon(aes(fill = value), size = 0.2) + xlab("") + ylab("") + coord_fixed(1) +
    annotate("text", x = max(center_locations$long) + 2.9, 
...
At the county level, strandings were not evenly distributed along the coast (*χ*^2^ = 1191.3, *p* < 0.001) (`r figs("countyfigure", display = 'cite')`). In Washington, strandings were highest in San Juan, Island, King, Pierce, and Grays Harbor counties. Similarly, the number of HI cases were significantly higher in Grays Harbor, Pierce, and Pacific counties, with a disproportionately higher number of cases in Pacific county compared with all strandings, the majority of which were gunshot wounds (Supplemental Table A). In Oregon, the majority of strandings occurred in Clatsop, Tillamook, Coos, and Lincoln counties (`r figs("countyfigure", display = 'cite')`). These counties were also where the majority of HI cases occurred, though a disproportionately higher number occurred in Clatsop, the majority of which were gunshot wounds (Supplemental Table B). 

<!-- Combined HI cases increased in Pierce (z = 3.0, *p* < 0.01), Pacific (z = 2.7, *p* < 0.01), King (z = 2.7, *p* < 0.01), Whatcom (z = 3.0, *p* < 0.001), and Jefferson (z = 2.2, *p* < 0.05) counties. Combined HI cases increased in Clatsop county (z = 3.9, *p* < 0.001).  -->
...
Each of the HI case types were concentrated in different areas, with boat collision injuries and fisheries interactions largely occurring throughout inland Washington waters and around the mouth of the Columbia River, while the only hotspot for gunshot wounds was centered around the Columbia River (`r figs("HI_type_facet", display = 'cite')`). These hotspots do shift seaonally. Boat collision cases during are most dense in inland Washington waters in the summmer and on the coast during winter. Fisheries and gunshot wound cases are only concentrated near Newport, OR during the spring but also exhibit clusters in Puget Sound the rest of the year (Supplemental Figure 1).
...
```{r, fig.height = 8.5, fig.width = 7, fig.align = 'center'}
countyfigure <- county_perc_diff
countyfigure
```

`r figs("countyfigure", caption = "Percent difference in the total number of human interaction cases in each county from the mean across all counties from 1989-2016. Points indicate locations of marine mammal stranding response networks.")`
